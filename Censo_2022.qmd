...
malha_2022 <- read_sf("analise_de_dados/dados/Censo 2022/DF_Malha_Preliminar_2022/DF_Malha_Preliminar_2022.shp")

malha_2010 <- read_sf("analise_de_dados/dados/Censo 2010/df_setores_censitarios/53SEE250GC_SIR.shp")

malha_2000_urbano <- read_sf("analise_de_dados/dados/Censo 2000/df_setores_censitarios_2000/Urbano/5300108.SHP")

malha_2000_rural <- read_sf("analise_de_dados/dados/Censo 2000/df_setores_censitarios_2000/Rural/53SE500G.shp")

regioes <- read_excel("analise_de_dados/dados/regioes.xlsx")
...
ggplot(malha_2022) +
  ...
  theme(panel.grid = element_line(colour = "transparent"),
        ...
        plot.caption = element_text(hjust = 0))+
  labs(x="", y="", title = "Número de Pessoas por Região Administrativa em 2022", caption = "Fonte: IBGE Censo 2022")
ggsave("Setores_pessoas.png", path = "graficos/", width = 190, height = 120, units = "mm")
```
...
![Malhas coloridas](Imagens\Malhas_coloridas.jpeg)
...
![Tipos de domíclios](Imagens\Tipos domícilios.jpeg)


[Fonte](https://agenciadenoticias.ibge.gov.br/media/com_mediaibge/arquivos/ba9e3c5e6ba08cd72e3d00f06d3886da.pdf)


Domicílio particular é a moradia onde o relacionamento entre seus ocupantes é ditado por laços de
parentesco, de dependência doméstica ou por normas de convivência.

Domicílio particular permanente é o domicílio que foi construído a fim de servir exclusivamente para habitação e,
na data de referência, tinha a finalidade de servir de moradia a uma ou mais pessoas.
...
Domicílio particular permanente ocupado sem entrevista é o domicílio particular permanente que estava ocupado na data de referência, porém não foi possível realizar a entrevista no momento da visita do Recenseador, já
que seus moradores estavam ausentes ou se recusaram a responder. Para estes domicílios, o IBGE utilizou uma metodologia de imputação, que se encontra descrita, adiante, no item Tratamento dos Dados.

Domicílio particular permanente de uso ocasional é o domicílio particular permanente que servia ocasionalmente de moradia na data de referência, ou seja, era o domicílio usado para descanso de fins de semana, férias ou
...
Domicílio particular improvisado ocupado é aquele domicílio que pode estar localizado em uma edificação que não tenha dependências destinadas exclusivamente à moradia (por exemplo, dentro de um bar), ou
em calçadas, praças ou viadutos, como também estruturas móveis ou abrigos naturais (como grutas ou cavernas) e que, na data de referência, estavam ocupados por moradores.
